...
const base=(id:string,insurer:Product['insurer'],name:string,category:Product['category'],scores:number[],strategic=false):Product=>{const pr=profile(category);return {id,insurer,name,category,summary:pr.summary,monthlyPremium:scores[0]*1800+35000,ageRange:category==='연금보험'?'30~70세':category==='어린이보험'?'0~30세':'20~65세',paymentPeriod:category==='정기보험'?'10/20년납':category==='연금보험'?'5/10/20년납':'10/20/30년납',coveragePeriod:category.includes('연금')?'종신 연금':category==='정기보험'?'10/20/30년 만기':'80세/100세/종신',deathScore:scores[1],healthScore:scores[2],criticalScore:scores[3],careScore:scores[4],pensionScore:scores[5],refundScore:scores[6],priceScore:scores[7],waiverScore:scores[8],clarityScore:scores[9],consultingScore:scores[10],hanwhaStrategic:strategic,strengths:[...pr.strengths],weaknesses:[...pr.weaknesses],recommendedFor:[...pr.recommendedFor],talkingPoints:[...pr.talkingPoints],differentiator:insurer==='한화생명'?`한화생명 ${category}은 고객 니즈와 상담 흐름을 연결하는 설명 포인트를 강조`:`${insurer} ${category}은 특정 비교 기준에서 대안으로 제시 가능한 상품`};};
